...
// traerFormNuevoClub   
   ...
   public function mostrarSubirRespaldo()
    ...
    public function subirRespaldo()
      ...
      $target_path = $target_path.basename( $this->files['uploadedfile']['name']); 

      $nombre = $this->files['uploadedfile']['name'];

      $pos1 = stripos($nombre, ".sql");

      if ($pos1===false)
              {             echo "
<script language='javascript' type='text/javascript'> 

    alert('el archivo no corresponde con el tipo.'); 
</script>  
            ";}
    else 
    {

        if(move_uploaded_file($this->files['uploadedfile']['tmp_name'], $target_path)) 
        {
          echo "El archivo ". basename( $this->files['uploadedfile']['name']). " ha sido subido";
...
<script language='javascript' type='text/javascript'> 
    ...
    alert('Respaldo subido con exito.'); 
</script>  
            ";
